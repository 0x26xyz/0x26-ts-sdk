import {
  Keypair,
  type IntentRequest,
  type OwnerCallRequest,
  signIntent,
  signOwnerCall,
  type SignedIntentJson,
  type SignedOwnerCallJson,
} from "./signing.js";
import type {
...
export type SuggestedLiquidationOutcome =
  | "clears_maintenance"
  | "still_under_maintenance"
  | "freezes_account"
  | "creates_bad_debt";
...
export interface ProduceBlockResponse {
  height: number;
  hash_hex: string;
...
export interface MatchFillResponse {
...
export interface SubmitOrderResponse {
  status:
    ...
    | "cancelled"
    | "resting";
  fills: MatchFillResponse[];
  filled_lots: number;
  remaining_lots: number;
  posted_order_id: number | null;
}
...
export interface OrderStatusResponse {
  ...
  filled_lots: number;
  remaining_lots: number;
  replacement_client_order_id: number | null;
...
export interface CancelOrderResponse {
  cancelled: boolean;
  order: RestingOrderResponse | null;
}
...
export interface FundingStateResponse {
  market_id: number;
...
export interface InsuranceFundResponse {
  market_id: number;
...
export interface MarketStateResponse {
  ...
  top_liquidation_accounts: AccountRiskSummaryResponse[];
}

export interface LiquidationPressureSummaryResponse {
  market_count: number;
  stressed_market_count: number;
  stressed_account_count: number;
  imminent_freeze_market_count: number;
  imminent_freeze_account_count: number;
  total_liquidation_candidate_count: number;
  total_liquidation_gap: string;
  worst_market_id: number | null;
  worst_market_gap: string;
  worst_account_id: number | null;
  worst_account_gap: string;
  suggested_clear_count: number;
  suggested_still_under_count: number;
  suggested_freeze_count: number;
  suggested_bad_debt_count: number;
  total_suggested_bad_debt: string;
  latest_market_risk_transition: MarketRiskTransition | null;
  recent_market_risk_transitions: MarketRiskTransition[];
  top_markets: MarketStateResponse[];
  top_accounts: AccountRiskSummaryResponse[];
}
...
export type AccountStreamEventName =
  | "account_trading_state"
  | "position"
  | "order"
  | "fill"
  | "collateral"
...
export interface SubmitSignedMarketMakerProgramActionParams {
  signed_market_maker_program_action: import("./signing.js").SignedMarketMakerProgramActionJson;
  wait_timeout_ms?: number;
}
...
export type RuntimeEventTopic =
  ...
  | "attestation_refreshed"
  | "attestation_revoked"
  | "attestation_expired"
  | "tier_changed"
  | "vendor_root_introduced"
  | "vendor_root_revoked"
  | "vendor_cap_profile_updated"
  | "tee_governance_action_rejected"
  | "real_capital_listing_upserted"
  | "real_capital_listing_deposit_enabled_updated"
  | "real_capital_listing_discovery_updated"
  | "real_capital_protocol_cap_updated"
  | "real_capital_global_kill_switch_updated"
  | "real_capital_governance_action_rejected";

export const DEFAULT_RUNTIME_EVENT_TOPICS: RuntimeEventTopic[] = [
  "account_created",
  "account_updated",
  "intent_accepted",
  "intent_promoted",
...
export class AndChainClient {
  ...
  constructor(
    ...
    this.marketDataUrl =
      options.marketDataUrl ?? `${this.baseUrl.replace(/\/$/, "")}/market-data`;
  }
  ...
  getAccount(accountId: number) {
    return this.call<AccountResponse>("kombat_getAccount", {
      account_id: accountId,
    });
  }
  ...
  submitOrder(order: Record<string, unknown>, timestampMs: number) {
    return this.call<SubmitOrderResponse>("kombat_submitOrder", {
      order,
      timestamp_ms: timestampMs,
    });
  }
  ...
  getOrderStatus(marketId: number, accountId: number, clientOrderId: number) {
    return this.call<OrderStatusResponse>("kombat_getOrderStatus", {
      market_id: marketId,
      account_id: accountId,
  ...
  cancelOrder(
    ...
    return this.call<CancelOrderResponse>("kombat_cancelOrder", {
      market_id: marketId,
      account_id: accountId,
      client_order_id: clientOrderId,
      timestamp_ms: timestampMs,
  ...
  replaceOrder(
    ...
    return this.call<SubmitOrderResponse>("kombat_replaceOrder", {
      market_id: marketId,
      account_id: accountId,
      client_order_id: clientOrderId,
      replacement,
  ...
  async getMarketDataCandles(params: {
    ...
    const url = new URL(
      `${this.marketDataUrl.replace(/\/$/, "")}/candles`,
      "http://localhost"
    );
    ...
    const requestUrl = /^https?:\/\//i.test(this.marketDataUrl)
      ? url.toString()
  ...
  getAccountTradingState(
    accountId: number,
    params: { marketId?: number; depth?: number; fillLimit?: number } = {}
  ) {
  ...
  getAccountPnlWindow(
    accountId: number,
    params: { fromMs: number; toMs?: number; marketId?: number }
  ) {
  ...
  getMarket(marketId: number) {
    return this.call<PerpetualMarketResponse | null>("kombat_getMarket", {
      market_id: marketId,
    });
  }

  listPerpetualMarkets(params: {
    status?: MarketStatus;
    limit?: number;
  } = {}) {
  ...
  getPosition(marketId: number, accountId: number) {
    return this.call<PositionResponse>("kombat_getPosition", {
      market_id: marketId,
      account_id: accountId,
    });
  }
  ...
  getFundingState(marketId: number) {
    return this.call<FundingStateResponse>("kombat_getFundingState", {
      market_id: marketId,
    });
  }

  getInsuranceFund(marketId: number) {
    return this.call<InsuranceFundResponse>("kombat_getInsuranceFund", {
      market_id: marketId,
    });
  }

  getLiquidationPressureSummary(params: {
    marketId?: number;
    outcome?: SuggestedLiquidationOutcome;
  } = {}) {
    return this.call<LiquidationPressureSummaryResponse>(
      "kombat_getLiquidationPressureSummary",
      {
        market_id: params.marketId,
  ...
  getAccountRiskSummary(
    ...
    return this.call<AccountRiskSummaryResponse>("kombat_getAccountRiskSummary", {
      account_id: accountId,
      market_id: params.marketId,
      outcome: params.outcome,
      include_flat: params.includeFlat,
  ...
  buildEventStreamUrl(query: EventStreamQuery = {}) {
    ...
    if (query.client_order_id != null) {
      url.searchParams.set("client_order_id", String(query.client_order_id));
    }
  ...
  openEventStream(
    ...
    const topics = query.topic ? [query.topic] : DEFAULT_RUNTIME_EVENT_TOPICS;
  ...
  openMarketStream(
    query: MarketStreamQuery,
    handlers: TypedStreamHandlers<MarketStreamEventName> = {}
  ) {
  ...
  openAccountStream(
    query: AccountStreamQuery,
    handlers: TypedStreamHandlers<AccountStreamEventName> = {}
  ) {
    return this.openTypedSseStream(
  ...
  openWalletStream(
    query: WalletStreamQuery,
    handlers: TypedStreamHandlers<WalletStreamEventName> = {}
  ) {
    return this.openTypedSseStream(
      this.buildWalletStreamUrl(query),
      ["wallet_balances"],
      handlers
    );
  }

  submitIntent(signedIntent: SignedIntentJson, timestampMs: number) {
  ...
  submitTypedIntent(
    ...
    return this.submitIntent(signIntent(signer, request), timestampMs);
  }

  submitOwnerCall(signedOwnerCall: SignedOwnerCallJson, timestampMs: number) {
  ...
  submitEvmOwnerCall(
    signedOwnerCall: Record<string, unknown>,
    timestampMs: number
  ) {
  ...
  submitTypedOwnerCall(
    signer: Keypair,
    request: OwnerCallRequest,
    timestampMs: number
  ) {
    return this.submitOwnerCall(signOwnerCall(signer, request), timestampMs);
  }
  ...
  submitSignedMarketMakerProgramAction(
    signedMarketMakerProgramAction: import("./signing.js").SignedMarketMakerProgramActionJson,
    waitTimeoutMs?: number
  ) {
    return this.call<MarketMakerProgramActionSubmissionResponse>(
  ...
  saveSnapshot(path: string) {
    return this.call<SnapshotFileResponse>("kombat_saveSnapshot", { path });
  }

  loadSnapshot(path: string) {
    return this.call<SnapshotFileResponse>("kombat_loadSnapshot", { path });
  }

  produceBlock(timestampMs: number, maxItems?: number) {
    return this.call<ProduceBlockResponse>("kombat_produceBlock", {
      timestamp_ms: timestampMs,
      max_items: maxItems,
    });
  }
